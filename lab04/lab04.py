...
class ArrayList:
    ...
    def append(self, value):
        ...
        ### BEGIN SOLUTION
        self.len += 1
        newlist = ConstrainedList(self.len)
        for i in range(self.len - 1):
            newlist[i] = self[i]
        newlist[self.len - 1] = value
        self.data = newlist
        ### END SOLUTION
    ...
    def pop(self, idx=-1):
        ...
        if idx < 0:
            idx = self.len + idx
        newlist = ConstrainedList(self.len - 1)
        for i in range(self.len - 1):
            if i < idx:
                newlist[i] = self[i]
            elif i >= idx:
    ...
    def __eq__(self, other):
        ...
        ### BEGIN SOLUTION
        if (type(other) != type(ArrayList())) | (len(other) != self.len):
            return False
        for i in range(self.len):
            if other[i] != self[i]:
                return False
    ...
    def __add__(self, other):
        ...
        ### BEGIN SOLUTION
        self.len += len(other)
        newlist = ArrayList(self.len)
        for i in range(self.len):
            if i < self.len - len(other):
                newlist[i] = self[i]
            else:
                newlist[i] = other[i - self.len + len(other)]
        return newlist
        ### END SOLUTION
