            ...
            if lc < len(self):
                if rc < len(self):
                    ...
                    if lcval > curval or rcval > curval:
                        if lcval > rcval:
                            sub = self.data[n]
                            self.data[n] = self.data[lc]
                            self.data[lc] = sub
                            self.heapify(lc)
                        else:
                            sub = self.data[n]
                            self.data[n] = self.data[rc]
                            self.data[rc] = sub
                            self.heapify(rc)
                else:
                    lcval = self.key(self.data[lc])
...
def running_medians(iterable):
    ...
    for i in iterable:
        if medians == []:
            medians.append(i)
            min_heap.add(i)
        elif len(medians) % 2 == 1:
            r = medians[len(medians) - 1]
            if max_heap.data == []:
                if min_heap.data[0] > int(i):
                ...
                max_heap.add(i)
            medians.append((max_heap.data[0] + min_heap.data[0]) / 2)
        else:
            r = medians[len(medians) - 1]
            if int(i) >= r:
                max_heap.add(i)
                medians.append(max_heap.data[0])
            else:
                min_heap.add(i)
                medians.append(min_heap.data[0])
    return medians
    ### END SOLUTION
...
def topk(items, k, keyf):
    ### BEGIN SOLUTION
    h = Heap(lambda x:-keyf(x))
    for i in items:
        if len(h) < k:
            h.add(i)
        elif keyf(i) > keyf(h.data[0]):
            h.pop()
            h.add(i)
    hf = []
    for i in h:
